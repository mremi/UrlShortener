...
namespace Mremi\UrlShortener\Provider\Firebase;
 ...
 */
class FirebaseProvider implements UrlShortenerProviderInterface
{
    /**
     * @var string
     */
     ...
     * Constructor.
     *
     * @param string $apiKey  A Firebase API key, optional
    ...
    public function shorten(LinkInterface $link)
    {
        ...
        $response = $client->post($this->getUri(), array_merge(
            array(
                'json' => array(
                    'longDynamicLink' => $link->getLongUrl(),
                ),
            ),
            $this->options
        ));
    ...
    protected function createClient()
    {
        return new Client(array(
            'base_uri' => 'https://firebasedynamiclinks.googleapis.com/v1/shortLinks',
        ));
    }
    ...
    /**
     * Validates the Firebase's response and returns it whether the status code is 200.
     *
    ...
    private function validate($apiRawResponse, $checkStatus = false)
    {
        ...
        if (null === $response) {
            throw new InvalidApiResponseException('Firebase response is probably mal-formed because cannot be json-decoded.');
        }

        if (property_exists($response, 'error')) {
            throw new InvalidApiResponseException(sprintf('Firebase returned status code "%s" with message "%s".',
        ...
        if (!property_exists($response, 'id')) {
            throw new InvalidApiResponseException('Property "id" does not exist within Firebase response.');
        }

        if (!property_exists($response, 'longUrl')) {
            throw new InvalidApiResponseException('Property "longUrl" does not exist within Firebase response.');
        }
        ...
        if (!property_exists($response, 'status')) {
            throw new InvalidApiResponseException('Property "status" does not exist within Firebase response.');
        }

        if ('OK' !== $response->status) {
            throw new InvalidApiResponseException(sprintf('Firebase returned status code "%s".', $response->status));
        }
